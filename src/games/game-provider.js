...
class GameProvider{
    ...
    selectSpy = async (nickname) => {
      ...
      const nickname = await this.gameRepo.selectSpy(nickname)
      
        let result = new Array()
      
        for(let i = 0; i < nickname.length; i++) {
         result.push(nickname[i])
        }
    ...
    giveWord = async (word) => {
        ...
        const giveWord = await this.gameRepo.giveWord(word)
    ...
    giveExample = async (category , word) => {
        ...
        const category = await this.gameRepo.giveExample(category)
        const word = shuffle(categoryFix)
    ...
    micToss = async (nickname) => {
      ...
      const micToss = await this.gameRepo.micToss(nickname)
        ...
        let randomSrart = result.slice(-1)

        console.log(randomSrart)
